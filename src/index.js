...
require('./cache').setCacheDirectory(resolve(popts.cache));
require('./fetch').setMapping(popts.host);

const log = new VieroLog('cdn');
...
require('./endpoints')
  .register(httpServer)
  .then(() => httpServer.run({ host: popts.address, port: popts.port }))
  .catch((err) => {
